...
async function createSocket() {
  ...
  socket.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      } else if (shouldReconnect && connectionStatus !== 'disconnected') {
...
export async function startWhatsAppConnection(): Promise<WhatsAppStatus> {
  if (isServerless()) {
    throw new AppError(
      'WhatsApp Web requires a persistent server. Run the API with npm run dev:api on a VPS or dedicated host.',
      503,
    );
  }
