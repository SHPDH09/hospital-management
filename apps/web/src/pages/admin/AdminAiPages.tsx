...
import { Bot, Settings, Zap, FileText, Send, Loader2, BarChart3, ShieldCheck, Plus, Trash2 } from 'lucide-react';
...
const subNav = [
  ...
  { to: '/admin/ai/approvals', label: 'Approvals', icon: ShieldCheck },
...
export function AdminAutomationPage() {
  ...
  const [form, setForm] = useState({
    ...
    actions: [{ type: 'send_notification', title: '', message: '' }],
    delayMinutes: 0,
  });
  ...
  const create = useMutation({
    mutationFn: () => api.post('/automation/rules', {
      ...
      actions: form.actions.map((a) => ({
        ...a,
        title: a.title || form.name,
        message: a.message || `Automation: ${form.name}`,
      })),
    }),
    onSuccess: () => { setShowForm(false); refetch(); },
  });

  const addCondition = () => setForm({ ...form, conditions: [...form.conditions, { field: '', operator: 'eq', value: '' }] });
  const removeCondition = (i: number) => setForm({ ...form, conditions: form.conditions.filter((_, idx) => idx !== i) });
  const addAction = () => setForm({ ...form, actions: [...form.actions, { type: 'alert_admin', title: '', message: '' }] });
  ...
  const TRIGGER_OPTIONS = ['appointment.created', 'appointment.upcoming', 'lead.created', 'payment.failed', 'review.created', 'verification.pending'];
  const OPERATORS = ['eq', 'neq', 'gt', 'gte', 'lt', 'lte', 'in'];
  const ACTION_TYPES = ['send_notification', 'send_reminder', 'alert_admin', 'assign_lead', 'create_task'];
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 overflow-y-auto">
          <div className="card p-6 w-full max-w-2xl space-y-4 my-8">
            <h3 className="font-semibold text-lg">Automation Builder — IF / THEN</h3>
            <input className="input w-full" placeholder="Rule name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-gray-500">WHEN (trigger)</label>
              ...
              <div>
                ...
                <input className="input w-full" placeholder="push,email,sms" value={form.channel} onChange={(e) => setForm({ ...form, channel: e.target.value })} />
              </div>
            </div>
            <div>
              <label className="text-xs text-gray-500">Delay (minutes)</label>
              <input type="number" className="input w-full" value={form.delayMinutes} onChange={(e) => setForm({ ...form, delayMinutes: Number(e.target.value) })} />
            </div>

            <div className="border rounded-lg p-3 bg-gray-50">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-medium">IF conditions</p>
                <button type="button" className="text-xs btn-ghost flex items-center gap-1" onClick={addCondition}><Plus className="h-3 w-3" /> Add</button>
              </div>
              {form.conditions.map((c, i) => (
                <div key={i} className="flex gap-2 mb-2 items-center">
                  <input className="input flex-1 text-sm" placeholder="field" value={c.field} onChange={(e) => {
                    const conditions = [...form.conditions];
                    conditions[i] = { ...c, field: e.target.value };
                    setForm({ ...form, conditions });
                  }} />
                  <select className="input w-24 text-sm" value={c.operator} onChange={(e) => {
                    const conditions = [...form.conditions];
                  ...
                  <input className="input flex-1 text-sm" placeholder="value" value={String(c.value)} onChange={(e) => {
                    const conditions = [...form.conditions];
            ...
            <div className="border rounded-lg p-3 bg-primary-50">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-medium">THEN actions</p>
                <button type="button" className="text-xs btn-ghost flex items-center gap-1" onClick={addAction}><Plus className="h-3 w-3" /> Add</button>
              </div>
              {form.actions.map((a, i) => (
                <div key={i} className="space-y-2 mb-3 pb-3 border-b border-primary-100 last:border-0">
                  <div className="flex gap-2 items-center">
                    <select className="input flex-1 text-sm" value={a.type} onChange={(e) => {
                    ...
                    <button type="button" onClick={() => removeAction(i)} className="text-red-500"><Trash2 className="h-4 w-4" /></button>
                  </div>
                  <input className="input w-full text-sm" placeholder="Title" value={a.title} onChange={(e) => {
                    const actions = [...form.actions];
                    actions[i] = { ...a, title: e.target.value };
                    setForm({ ...form, actions });
                  }} />
                  <input className="input w-full text-sm" placeholder="Message" value={a.message} onChange={(e) => {
                    const actions = [...form.actions];
                    actions[i] = { ...a, message: e.target.value };
                    setForm({ ...form, actions });
                  }} />
                </div>
              ))}
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button type="button" className="btn-primary" onClick={() => create.mutate()} disabled={!form.name}>Create Automation</button>
            </div>
          </div>
        </div>
      )}
...
function FraudInsightsSection() {
  const { data } = useQuery({
    queryKey: ['ai-fraud'],
    queryFn: () => api.get<{
      totalFlags: number;
      highSeverity: number;
      reviews: { flags: { type: string; severity: string; message: string }[]; insight?: string };
      referrals: { flags: { type: string; severity: string; message: string }[]; insight?: string };
    }>('/ai/fraud'),
  });
  const f = data?.data;
  if (!f) return null;
  ...
  return (
    <section className="card p-6">
      <h2 className="font-semibold mb-2">Fraud Detection</h2>
      <p className="text-sm mb-3">{f.totalFlags} flags ({f.highSeverity} high severity)</p>
      {f.reviews.insight && <p className="text-sm text-gray-600 mb-2">Reviews: {f.reviews.insight}</p>}
      {f.referrals.insight && <p className="text-sm text-gray-600 mb-2">Referrals: {f.referrals.insight}</p>}
      {[...f.reviews.flags, ...f.referrals.flags].slice(0, 8).map((flag, i) => (
        <div key={i} className="text-sm border-l-4 border-red-400 pl-3 py-1 mb-2">[{flag.severity}] {flag.message}</div>
      ))}
    </section>
  );
}
...
export function AdminApprovalsPage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () => api.get('/ai/approvals?status=PENDING'),
  });
  const { data: stats } = useQuery({
    queryKey: ['ai-approval-stats'],
    queryFn: () => api.get<{ pending: number }>('/ai/approvals/stats'),
  });

  const review = useMutation({
    mutationFn: ({ id, decision, notes }: { id: string; decision: 'APPROVED' | 'REJECTED'; notes?: string }) =>
      api.post(`/ai/approvals/${id}/review`, { decision, notes }),
    onSuccess: () => { refetch(); qc.invalidateQueries({ queryKey: ['ai-approval-stats'] }); },
  });
  ...
  return (
    <DashboardLayout portal="admin">
      <PageHeader title="AI Approval Queue" subtitle={`${stats?.data?.pending ?? 0} pending — human review required for AI recommendations`} />
      <AiSubNav />
      {isLoading ? <LoadingState /> : rows.length === 0 ? (
        <div className="card p-8 text-center text-gray-500">
          <p>No pending approval requests.</p>
          <p className="text-sm mt-2">Run document verification analysis from Hospitals page to create approval requests.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {rows.map((r) => {
            const payload = r.payload as { aiSummary?: string; recommendation?: string; completenessScore?: number; organizationName?: string } | undefined;
            return (
              <div key={r.id as string} className="card p-5">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="font-medium">{String(r.actionType)} — {String(r.entityType)}</p>
                    <p className="text-sm text-gray-500 mt-1">{payload?.aiSummary || 'No summary'}</p>
                    {payload?.completenessScore !== undefined && (
                      <p className="text-xs mt-2">Completeness: {payload.completenessScore}% · Recommendation: <StatusBadge status={payload.recommendation === 'RECOMMEND_APPROVE' ? 'ACTIVE' : 'PENDING'} /></p>
                    )}
                    <p className="text-xs text-gray-400 mt-1">{new Date(r.createdAt as string).toLocaleString()}</p>
                  </div>
                  <div className="flex gap-2 shrink-0">
                    <ActionBtn variant="success" onClick={() => review.mutate({ id: r.id as string, decision: 'APPROVED' })}>Approve</ActionBtn>
                    <ActionBtn variant="danger" onClick={() => review.mutate({ id: r.id as string, decision: 'REJECTED' })}>Reject</ActionBtn>
                  </div>
                </div>
              </div>
            );
