...
interface RenewalPayment {
  id: string; amount: number; currency: string; billingCycle: string; status: string;
  invoiceNumber?: string | null; method?: string | null; createdAt: string; paidAt?: string | null;
}

function loadCashfreeSdk(): Promise<((opts: { mode: string }) => { checkout: (o: Record<string, unknown>) => void }) | null> {
  ...
  const renewNow = async () => {
    ...
    try {
      ...
      if (sessionId) {
        ...
        setNotice({ kind: 'success', text: `Renewal order ${res.data?.invoiceNumber ?? ''} created for ${formatCurrency(res.data?.amount ?? 0)}.` });
      }
      refetch();
    } finally {
      setBusy(false);
      ...
      {isLoading ? <LoadingState /> : !sub ? (
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className={`card p-6 ${expiringSoon ? 'ring-1 ring-amber-300' : ''}`}>
              ...
              <div className="mt-4 grid grid-cols-2 gap-4 text-sm sm:grid-cols-3">
                <div><p className="text-xs uppercase tracking-wide text-gray-400">Start</p><p className="mt-0.5 font-medium">{new Date(String(sub.startDate)).toLocaleDateString()}</p></div>
                {sub.endDate ? <div><p className="text-xs uppercase tracking-wide text-gray-400">Expires</p><p className="mt-0.5 font-medium">{new Date(String(sub.endDate)).toLocaleDateString()}</p></div> : null}
                <div>
                  <p className="text-xs uppercase tracking-wide text-gray-400">Days remaining</p>
                  <p className={`mt-0.5 font-semibold ${expiringSoon ? 'text-amber-600' : 'text-gray-800'}`}>{daysRemaining != null ? `${daysRemaining} days` : '—'}</p>
                </div>
              </div>
            ...
            <div className="card overflow-hidden">
              ...
              {payments.length === 0 ? (
                <div className="p-8 text-center text-sm text-gray-500">No renewal transactions yet</div>
              ) : (
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 text-left text-xs uppercase tracking-wide text-gray-500">
                    <tr><th className="px-6 py-3 font-medium">Invoice</th><th className="px-6 py-3 font-medium">Amount</th><th className="px-6 py-3 font-medium">Cycle</th><th className="px-6 py-3 font-medium">Status</th><th className="px-6 py-3 font-medium">Date</th></tr>
                  </thead>
                  <tbody>
                    {payments.map((p) => (
                      <tr key={p.id} className="border-t border-gray-100">
                        <td className="px-6 py-3 font-medium text-gray-800">{p.invoiceNumber || '—'}</td>
                        <td className="px-6 py-3">{formatCurrency(p.amount)}</td>
                        <td className="px-6 py-3">{p.billingCycle}</td>
                        <td className="px-6 py-3"><StatusBadge status={p.status} /></td>
                        <td className="px-6 py-3 text-gray-500">{new Date(p.paidAt || p.createdAt).toLocaleDateString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="space-y-6">
            <div className="card p-6">
              ...
              <p className="mt-1 text-sm text-gray-500">Automatically attempt payment before your plan expires.</p>
              <button
                onClick={toggleAutoRenew}
                className={`mt-4 inline-flex h-6 w-11 items-center rounded-full transition-colors ${payload?.autoRenew ? 'bg-primary-600' : 'bg-gray-300'}`}
              >
                <span className={`inline-block h-5 w-5 transform rounded-full bg-white shadow transition-transform ${payload?.autoRenew ? 'translate-x-5' : 'translate-x-1'}`} />
              </button>
