...
const adminNav = [
  ...
  { to: '/admin/organizations', icon: Heart, label: 'Organizations' },
  { to: '/admin/subscriptions', icon: Receipt, label: 'Subscriptions' },
  { to: '/admin/advertisements', icon: Settings, label: 'Advertisements' },
];
...
export function DashboardLayout({ children, portal }: DashboardLayoutProps) {
  ...
  const nav = portal === 'crm' ? crmNav : portal === 'admin' ? adminNav : patientNav;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-gray-200 bg-white px-4 lg:px-8">
          ...
          <Link to="/" className="text-sm text-gray-500 hover:text-primary-600 ml-auto">
            Back to Website
          </Link>
        </header>
